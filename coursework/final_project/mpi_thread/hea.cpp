...
pthread_mutex_t mutexNetEnd = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutexCount = PTHREAD_MUTEX_INITIALIZER;

typedef vector<Chromosome> vect_chrom_type;
Data* data;

void printResult(double fitness, int numProcs, int numThreads, int found, int numGen, double time, int num_chromosomes, int num_elite_set, int num_generations);
...
void *run(void *arguments) {
    ...
    int myEnd = 0;
    pthread_mutex_lock(&mutexNetEnd);
    int netEndAux = netEnd;
    pthread_mutex_unlock(&mutexNetEnd);
    ...
    while (netEndAux + threadEndAux < args->num_procs + NTHREADS) {
        ...
        if (args->my_thread_id == 0) {
            ...
            if (stop == false && threadEndAux == NTHREADS) {
                ...
                if (setting->verbose) {
                    printf(
                            "All P%1$d threads are completed\n",
                            args->my_rank);
                }
            }
            ...
            do {
                MPI_Iprobe(MPI_ANY_SOURCE, TAGEND, MPI_COMM_WORLD, &flag, &args->status);
                if (flag) {
                    int temp;
                    MPI_Recv(&temp, 1, MPI_INT, MPI_ANY_SOURCE, TAGEND, MPI_COMM_WORLD, &args->status);
                    pthread_mutex_lock(&mutexNetEnd);
                    netEnd += temp;
                    netEndAux = netEnd;
                    pthread_mutex_unlock(&mutexNetEnd);
            ...
            int count_aux;
            pthread_mutex_lock(&mutexCount);
            count_aux = global_gen_count;
            pthread_mutex_unlock(&mutexCount);
            pthread_mutex_lock(&mutexBest);
            if ((count_aux % interval == 0) || (((global_best.fitness / 100) * x) < (local_best - global_best.fitness))) {
                for (int i = 0; i < args->num_procs; i++) {
                    if (i != args->my_rank) {
            ...
            do {
                MPI_Iprobe(MPI_ANY_SOURCE, TAGBEST, MPI_COMM_WORLD, &flag, &args->status);
                if (flag) {
                    double aux_best;
                    MPI_Recv(&aux_best, 1, MPI_DOUBLE, MPI_ANY_SOURCE, TAGBEST, MPI_COMM_WORLD, &args->status);
                    pthread_mutex_lock(&mutexBest);
                    if (aux_best < global_best.fitness) {
                        int aux = 0;
                        MPI_Send(&aux, 1, MPI_INT, args->status.MPI_SOURCE, TAGREQ, MPI_COMM_WORLD);
                    }
                    pthread_mutex_unlock(&mutexBest);
                }
            } while (flag);
            ...
            do {
                MPI_Iprobe(MPI_ANY_SOURCE, TAGREQ, MPI_COMM_WORLD, &flag, &args->status);
                if (flag) {
                    int aux;
                    MPI_Recv(&aux, 1, MPI_INT, MPI_ANY_SOURCE, TAGREQ, MPI_COMM_WORLD, &args->status);
                    ...
                    // double
                    MPI_Pack(&global_best.fitness, 1, MPI_DOUBLE, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(&global_best.lambda, 1, MPI_DOUBLE, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(&global_best.transfer_size, 1, MPI_DOUBLE, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);

                    // int vector
                    int alloc_size = global_best.allocation.size();
                    int height_size = global_best.height_soft.size();
                    int order_size = global_best.ordering.size();
                    MPI_Pack(&alloc_size, 1, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(global_best.allocation.data(), alloc_size, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(&height_size, 1, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(global_best.height_soft.data(), height_size, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(&order_size, 1, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(global_best.ordering.data(), order_size, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);

                    // double vector
                    int time_size = global_best.time_vector.size();
                    int start_size = global_best.start_time_vector.size();
                    MPI_Pack(&time_size, 1, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(global_best.time_vector.data(), time_size, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(&start_size, 1, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);
                    MPI_Pack(global_best.start_time_vector.data(), start_size, MPI_INT, buffer, BUFFERSIZE, &position, MPI_COMM_WORLD);

                    // send pack
                    MPI_Send(buffer, BUFFERSIZE, MPI_PACKED, args->status.MPI_SOURCE, TAGCROMO, MPI_COMM_WORLD);
                    pthread_mutex_unlock(&mutexBest);
                }
            } while (flag);
            ...
            //TODO
            do {
                MPI_Iprobe(MPI_ANY_SOURCE, TAGCROMO, MPI_COMM_WORLD, &flag, &args->status);
                if (flag) {
                    //packing
                    position = 0;
                    Chromosome aux;
                    MPI_Recv(&buffer, BUFFERSIZE, MPI_PACKED, MPI_ANY_SOURCE, TAGCROMO, MPI_COMM_WORLD, &args->status);
                    ...
                    pthread_mutex_lock(&mutexBest);
                    if (global_best.fitness > aux.fitness) {
        ...
        if (setting->verbose && (count % setting->print_gen) == 0) {
            pthread_mutex_lock(&mutexBest);
            cout << "|P" << args->my_rank << "|T" << args->my_thread_id << "|" << "Gen: " << count << " Fitness: " << global_best.fitness / 60.0 << "(s)" << endl;
            pthread_mutex_unlock(&mutexBest);
        }
        ...
        if (count == setting->num_generations && myEnd == 0 || time_running > MAX_TIME && myEnd == 0) {
            ...
            if (setting->verbose) {
                printf(
                        "|P%1$d|T%2$d| Generations completed\n",
                        args->my_rank, args->my_thread_id);
            }
        }
    }
    if (setting->verbose) {
        printf(
                "\n|P%1$d|T%2$d| Stopping\n",
                args->my_rank, args->my_thread_id);
    }
...
int main(int argc, char **argv) {
    ...
    // printf("a Process %d on %s out of %d\n", rank, processor_name, numprocs);
    ...
    for (int i = 0; i < NTHREADS; i++) {
        ...
        args[i].num_procs = numprocs;
        args[i].status = status;
        // args[i].local_generations = local_generations;
    ...
    // global_best = run(&name_workflow, &name_cluster);
    global_best.computeFitness(true, true);

    int printing = false;

    if (rank != master) {
        MPI_Send(&global_best.fitness, 1, MPI_DOUBLE, master, tag, world);
    } else {

        // Compare all fitnesses
        double fitnesses[numprocs];
        fitnesses[master] = global_best.fitness;
    ...
    if (printing == true) {
        ...
        printResult(name_workflow, global_best.fitness, foundSeqGen, elapseTimeFoundSecs, global_gen_count, elapseSecs, setting->num_chromosomes, setting->num_elite_set, setting->num_generations, foundMyGen, elapseMyTimeFoundSecs, numprocs, NTHREADS);
...
void printResult(string arquivo, double fitness, int found, double time_found, int total, double time, int num_chromosomes, int num_elite_set, int num_generations, int foundMyGen, double foundMyTime, int numProcs = 1, int numThreads = 1) {
    ...
    std::ofstream out;
    out.open("csv/mpi_thread.csv", std::ios::app);
